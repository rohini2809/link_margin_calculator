...
# ============================================================
# SECTION 2 — USE CASE PRESET PARAMETERS (from Link Budget PDF)
# ============================================================
USE_CASES = {
...
def calc_slant_range(elevation_deg, altitude_km):
    """
    Correct slant range geometry.
    At El=90 (zenith): returns exactly altitude_km.
    At El=10 (horizon): returns longer path through atmosphere.
    """
    El   = np.radians(elevation_deg)
    Re   = EARTH_RADIUS_KM
    h    = altitude_km
    slant = np.sqrt((Re * np.cos(El))**2 + h**2 + 2*Re*h) - Re * np.cos(El)
    return slant

def calc_fspl_dB(slant_range_km, frequency_GHz):
    """Free Space Path Loss — pure geometric spreading of signal."""
...
def calc_antenna_gain_dB(diameter_m, frequency_GHz, efficiency):
    """Parabolic dish gain from physical size and frequency."""
    f          = frequency_GHz * 1e9
...
def calc_beamwidth_deg(diameter_m, frequency_GHz):
    """3dB beamwidth — how wide the antenna 'sees'. Bigger dish = narrower beam."""
    f          = frequency_GHz * 1e9
    wavelength = SPEED_OF_LIGHT / f
...
def calc_zenith_pointing_loss_dB(elevation_deg, beamwidth_deg):
    ...
    At El=90 (directly above): loss = 0 dB (perfect alignment)
    At El=10 (near horizon): loss is large (antenna misses satellite badly)
    """
...
def calc_pass_duration_minutes(min_viable_el, altitude_km):
    """
    Estimate comms duration per pass.
    Uses orbital geometry: how long satellite spends above min_viable_el.
    """
    ...
    orbital_r   = Re + h
    orbital_vel = np.sqrt(mu / orbital_r)   # km/s
    orbital_period_s = 2 * np.pi * orbital_r / orbital_vel

    # Half-angle of visibility from ground at min elevation
    El_rad      = np.radians(min_viable_el)
    rho         = np.arccos(Re * np.cos(El_rad) / orbital_r) - El_rad
    # Full arc visible = 2 * rho
    fraction    = (2 * rho) / (2 * np.pi)
    duration_s  = fraction * orbital_period_s
    return duration_s / 60.0   # convert to minutes
...
def calc_link_margin(elevation_deg, eirp_dBW, gt_dBK,
    ...
    antenna_mode: 'non_steerable' or 'steerable'
    """
    # Slant range
    ...
    if antenna_mode == "non_steerable":
        ...
        pointing_loss  = 0.0   # steerable — always on target
    ...
    total_losses = (fspl_dB
                    ...
                    + rain_loss_dB
                    + fixed_pointing_loss_dB
                    + abs(pointing_loss))   # pointing loss is negative, abs for subtraction
...
def find_min_antenna_size(params, data_rate_bps, antenna_mode, frequency_GHz=2.4):
    for d in np.arange(0.1, 10.1, 0.05):
        lm_at_min_el, *_ = calc_link_margin(
            MIN_ELEVATION_DEG,
            params["eirp_dBW"],
            params["gt_dBK"],
            data_rate_bps,
...
This tool evaluates link margin across elevation angles for three use cases:
TC Uplink, TM Downlink, and Payload Downlink —
for both the fixed zenith 0.7m antenna and a steerable antenna.
""")
...
# Use case selector
use_case_name = st.sidebar.selectbox(
    "📋 Select Use Case",
    list(USE_CASES.keys())
)
...
antenna_mode = st.sidebar.radio(
    ...
    format_func=lambda x: "🔒 Non-Steerable (Fixed Zenith)" if x == "non_steerable" else "🎯 Steerable (Tracking)"
)
...
st.sidebar.caption("Defaults loaded from MangoSat Link Budget PDF")
...
min_dish           = find_min_antenna_size(params_live, params_live["desired_data_rate_bps"], antenna_mode)
...
st.caption("All values at desired data rate unless stated. Reference margin from MangoSat Link Budget PDF.")

col1, col2, col3, col4, col5 = st.columns(5)

with col1:
    st.metric("Link Margin @ 10° (Desired DR)",
              f"{lm_at_10deg:.1f} dB",
...
with col3:
    st.metric("Pass Duration (Desired DR)",
              ...
              delta=f"Min elevation: {min_viable_desired}°" if min_viable_desired else "Link never closes")
with col4:
    st.metric("Pass Duration @ 2400 bps",
              f"{pass_dur_min_dr:.1f} min" if pass_dur_min_dr > 0 else "No link",
              delta=f"Min elevation: {min_viable_min_dr}°" if min_viable_min_dr else "Link never closes")
with col5:
...
st.markdown(f"""
> 📄 **Reference check:** PDF shows link margin of **{params['ref_margin_dB']} dB** at reference elevation.
> Your current zenith margin is **{lm_at_90deg:.1f} dB**.
""")
...
st.markdown(f"""
**Blue line** = margin at desired data rate ({params_live['desired_data_rate_bps']:,} bps) &nbsp;|&nbsp;
**Orange dashed** = margin at minimum data rate (2,400 bps) &nbsp;|&nbsp;
...
# ============================================================
# SECTION 9 — LOSS BREAKDOWN
# ============================================================
st.markdown("---")
st.subheader("🔍 Loss Breakdown vs Elevation Angle")
st.markdown("""
- **Slant Range** — actual signal travel distance. Increases at low elevation (satellite is farther away geometrically).
- **FSPL** — Free Space Path Loss. Directly tied to slant range — more distance = more spreading = more loss.
- **Pointing Loss** — only applies to non-steerable mode. Fixed zenith antenna misses satellite at low elevation. Zero for steerable.
""")
...
axes[0].annotate(f"At zenith: {ranges[-1]:.0f} km ≈ {ALTITUDE_KM} km ✓",
                 xy=(85, ranges[-1]), fontsize=8, color='green')
...
axes[2].set_title(f"Pointing Loss (dB)\n({'Fixed Zenith' if antenna_mode == 'non_steerable' else 'Steerable = 0 dB always'})")
...
st.markdown("""
Shows how link margin changes across both elevation angle and dish size.
**Green = link OK, Red = link failed.**
The white dashed line marks the current 0.7m MangoSat antenna.
Use this to answer: *"How big does the dish need to be?"*
""")
...
im = ax3.imshow(heatmap1, aspect='auto', origin='lower',
                extent=[elevs_hm[0], elevs_hm[-1], diameters_hm[0], diameters_hm[-1]],
                cmap='RdYlGn', norm=norm)
plt.colorbar(im, ax=ax3, label='Link Margin (dB)')
ax3.axhline(y=0.7, color='white', linestyle='--', linewidth=2, label='Current 0.7m antenna')
...
st.markdown("""
Shows sensitivity to transmit power.
Useful for asking: *"What if the satellite transmit power degrades on orbit?"*
The white dashed line marks the current EIRP from the link budget.
""")
...
im2 = ax4.imshow(heatmap2, aspect='auto', origin='lower',
                 extent=[elevs_hm[0], elevs_hm[-1], eirps_hm[0], eirps_hm[-1]],
...
# --- Heatmap 3: Desired DR vs Elevation ---
st.markdown("---")
st.subheader("🌡️ Heatmap 3: Link Margin vs Elevation & Data Rate")
st.markdown("""
Higher data rate = harder to close the link.
This heatmap shows what data rates are achievable at each elevation angle.
Use this to answer: *"What is the maximum data rate I can use at 20° elevation?"*
""")
...
# ============================================================
# SECTION 11 — PASS DURATION SUMMARY
# ============================================================
st.markdown("---")
st.subheader("⏱️ Comms Duration Per Pass Summary")
st.markdown("""
Based on orbital geometry at 590 km altitude.
Shows how many minutes per pass the link is viable — at desired and minimum data rates.
""")

col_a, col_b = st.columns(2)
with col_a:
    st.info(f"""
    **At Desired Data Rate ({params_live['desired_data_rate_bps']:,} bps)**
    - Min viable elevation: **{min_viable_desired}°** {'✅' if min_viable_desired else '❌'}
    - Estimated pass duration: **{pass_dur_desired:.1f} minutes**
    """)
with col_b:
    st.info(f"""
    **At Minimum Data Rate (2,400 bps)**
    - Min viable elevation: **{min_viable_min_dr}°** {'✅' if min_viable_min_dr else '❌'}
    - Estimated pass duration: **{pass_dur_min_dr:.1f} minutes**
    """)
...
table = pd.DataFrame({
    ...
    f"Margin @ {params_live['desired_data_rate_bps']:,} bps (dB)": margins_desired_full[idx].round(2),
...
# ============================================================
# SECTION 13 — PARAMETER REFERENCE
# ============================================================
st.markdown("---")
with st.expander("📄 View Full Parameter Reference (from MangoSat Link Budget PDF)"):
    st.markdown(f"""
    | Parameter | Value |
    |---|---|
    ...
    | Pointing Loss (fixed) | {params['pointing_loss_dB']} dB |
    ...
    | **Reference Link Margin (PDF)** | **{params['ref_margin_dB']} dB** |
    """)